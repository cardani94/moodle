...
class behat_question_base extends behat_base {
    ...
    protected function finish_adding_question($questiontypename, TableNode $questiondata) {
        ...
        $this->execute_step('behat_forms::i_set_the_field_to', array($this->escape($questiontypename), 1),
            false, false);
        $this->execute_step("behat_general::i_click_on", array('.submitbutton', "css_element"),
            true, true);

        $this->execute_step("behat_forms::i_set_the_following_fields_to_these_values", $questiondata,
            false, false);
        $this->execute_step("behat_forms::press_button", 'id_submitbutton',
            false, false);
    }
}
