...
class behat_grade extends behat_base {
    ...
    public function i_give_the_grade($grade, $userfullname, $itemname) {
        ...
        $this->execute_step('behat_forms::i_set_the_field_to', array($this->escape($fieldstr), $grade),
            false, false);
    }
    ...
    public function i_give_the_feedback($feedback, $userfullname, $itemname) {
        ...
        $this->execute_step('behat_forms::i_set_the_field_to', array($this->escape($fieldstr), $this->escape($feedback)),
            false, false);
    }
    ...
    public function i_set_the_following_settings_for_grade_item($gradeitem, TableNode $data) {
        ...
        if ($this->running_javascript()) {
            ...
            if ($this->getSession()->getPage()->findAll('xpath', $xpath)) {
                $this->execute_step("behat_general::i_click_on", array($this->escape($xpath), "xpath_element"),
                    true, true);
            }
        }
        ...
        $this->execute_step("behat_general::i_click_on", array($this->escape($linkxpath), "xpath_element"),
            true, true);
        $this->execute_step("behat_forms::i_set_the_following_fields_to_these_values", $data,
            true, true);
        $this->execute_step('behat_forms::press_button', $this->escape($savechanges),
            false, false);
    }
    ...
    public function i_set_calculation_for_grade_item_with_idnumbers($calculation, $gradeitem, TableNode $data) {
        ...
        foreach ($datahash as $gradeitem => $idnumber) {
            ...
            "]";
            $this->execute_step('behat_forms::i_set_the_field_with_xpath_to', array($inputxpath, $idnumber),
                true, true);
        }

        $this->execute_step('behat_forms::press_button', get_string('addidnumbers', 'grades'),
            true, true);
        $this->execute_step('behat_forms::i_set_the_field_to', array(get_string('calculation', 'grades'), $calculation),
            true, true);
        $this->execute_step('behat_forms::press_button', $savechanges,
            false, false);
    ...
    public function i_reset_weights_for_grade_category($gradeitem) {
        ...
        $this->execute_step("behat_general::i_click_on", array($this->escape($linktext), "link"),
            false, false);
    }
