...
class behat_grading extends behat_base {
    ...
    public function i_go_to_advanced_grading_page($activityname) {
        ...
        $this->execute_step('behat_general::click_link', $this->escape($activityname),
            true, true);

        $this->execute_step('behat_general::click_link', get_string('gradingmanagement', 'grading'),
            false, false);
    }
    ...
    public function i_go_to_advanced_grading_definition_page($activityname) {
        ...
        $this->execute_step('behat_grading::i_go_to_advanced_grading_page', $this->escape($activityname),
            true, true);

        $this->execute_step("behat_general::i_click_on", array($this->escape($definitionxpath), "xpath_element"),
            false, false);
    }
    /**
    ...
    public function i_go_to_activity_advanced_grading_page($userfullname, $activityname) {
        ...
        if ($this->getSession()->getPage()->find('named_partial', array('link', $usergradetextliteral))) {
            $this->execute_step('behat_general::click_link', $this->escape($usergradetext),
                false, false);
            return true;
        }
        ...
        $this->execute_step('behat_general::click_link', $this->escape(get_string('viewgrading', 'assign')),
            true, true);
    ...
    public function i_publish_grading_form_definition_as_a_public_template($activityname) {
        ...
        $this->execute_step("behat_general::i_click_on", array($this->escape(get_string("manageactionshare", "grading")), "link"),
            true, true);

        $this->execute_step('behat_forms::press_button', get_string('continue'),
            false, false);
    }
    ...
    public function i_set_activity_to_use_grading_form($activityname, $templatename) {
        ...
        $this->execute_step('behat_general::click_link', $this->escape(get_string('manageactionclone', 'grading')),
            true, true);
        $this->execute_step('behat_forms::i_set_the_field_to', array(get_string('searchownforms', 'grading'), 1),
            true, true);
    ...
    public function i_complete_the_advanced_grading_form_with_these_values(TableNode $data) {
        $this->execute_step("behat_forms::i_set_the_following_fields_to_these_values", $data,
            true, true);
        $this->execute_step('behat_grading::i_save_the_advanced_grading_form', false, false);
    }
}
