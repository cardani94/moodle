...
abstract class format_section_renderer_base extends plugin_renderer_base {
    ...
    protected function section_summary($section, $course) {
        ...
        $dimsection = '';
        If (!$section->visible) {
            $dimsection = ' dimmed_text';
        }

        $o = '';
        $o.= html_writer::start_tag('li', array('id' => 'section-'.$section->section,
            'class' => 'section-summary clearfix'.$dimsection));
    ...
    protected function get_nav_links($course, $sections, $sectionno) {
        ...
        while ($back > 0 and empty($links['previous'])) {
            if ($canviewhidden || $sections[$back]->visible) {
                ...
                if (!$sections[$back]->visible) {
                    $dimlink = array('class' => 'dimmed_text');
                }
                $previouslink = html_writer::tag('span', $this->output->larrow(), array('class' => 'larrow'));
                $previouslink .= get_section_name($course, $sections[$back]);
                $links['previous'] = html_writer::link(course_get_url($course, $back), $previouslink, $dimlink);
            }
            $back--;
        }
        ...
        while ($forward <= $course->numsections and empty($links['next'])) {
            if ($canviewhidden || $sections[$forward]->visible) {
                ...
                $links['next'] = html_writer::link(course_get_url($course, $forward), $nextlink, $dimlink);
            }
            $forward++;
        }
    ...
    public function print_single_section_page($course, $sections, $mods, $modnames, $modnamesused, $displaysection) {
        ...
        $sectiontitle .= html_writer::tag('div', get_section_name($course, $sections[$displaysection]), $titleattr);
