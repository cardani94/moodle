...
if ($chooselog) {
    ...
    $event = \report_log\event\content_viewed::create(array('courseid' => $id,
                                                            'other'    => array('content' => 'logs')));
    $event->set_page_detail();
    $event->set_legacy_logdata(array($id, "course", "report log", "report/log/index.php?id=$id", $id));
    $event->trigger();
}
    ...
    if (!empty($chooselog)) {
        switch ($logformat) {
            case 'showashtml':
                ...
                break;
            case 'downloadascsv':
                $reportlog->print_log_csv();
                exit();
            case 'downloadasods':
                $reportlog->print_log_ods();
                exit();
            case 'downloadasexcel':
                $reportlog->print_log_xls();
                exit();
        }
    } else {
        echo $output->header();
