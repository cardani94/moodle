...
class report_log_renderer extends plugin_renderer_base {
    ...
    public function render_report_log_renderable(report_log_renderable $reportlog) {
        ...
        if ($reportlog->showreport) {
            $table = $reportlog->setup_table();
            $logdata = $reportlog->get_logs();

            $this->log_report($reportlog, $table, $logdata);
        }
    }
     ...
     *
     * @param report_log_renderable $reportlog object of report_log.
     * @param flexible_table $table table to add records to.
     * @param array $logdata logdata containing totalcount and log data.
     */
    public function log_report(report_log_renderable $reportlog, \flexible_table $table, $logdata) {
    ...
    public function report_selector_form(report_log_renderable $reportlog) {
            ...
            if ($reportlog->showcourses) {
                ...
                if (has_capability('report/log:view', $sitecontext)) {
                    ...
                    $a->url = new moodle_url('/report/log/index.php', array('chooselog' => 0,
                        ...
                        'id' => $selectedcourseid, 'date' => $reportlog->selecteddate, 'modid' => $reportlog->selectedmodid,
        ...
        $logformats = $reportlog->get_log_formats();
        echo html_writer::label(get_string('logsformat', 'report_log'), 'menulogformat', false, array('class' => 'accesshide'));
        echo html_writer::select($logformats, 'logformat', $reportlog->selectedlogformat, false);
