...
Feature: In a report, admin can filter log data
  ...
  Scenario: Filter log report for standard log reader
    ...
    And I click on "a[href$='store=logstore_standard']" "css_element"
    And I follow "Home"
    ...
    And I select "Site logs" from "id"
    And I select "All participants" from "user"
    And I select "Standard log" from "logreader"
    And I press "Get these logs"
    Then I should see "User logged in as another user"
    And I select "Legacy log" from "logreader"
    And I press "Get these logs"
