...
class behat_navigation extends behat_base {
    ...
    /**
     * Click link in navigation tree that matches the text in parentnode/s (comma separated if more then one)
     * @Given /^I navigate to "(?P<nodetext_string>(?:[^"]|\\")*)" node in "(?P<parentnodes_string>(?:[^"]|\\")*)"$/
     *
     * @throws ExpectationException
     * @param string $nodetext navigation node to click.
     * @param string $parentnodes comma separated list of parent nodes.
     */
