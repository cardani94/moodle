...
/** Show/hide all blocks on page */
M.util.init_hideallblocks = function(Y, hideallblocks, showallblocks) {
    var hideblocknode = Y.one('#hideallblocks');
    if (!hideblocknode) {
        return false;
    }

    Y.use('event',function(altstr){
        var SELECTORS = {
            MAINREGION: '#region-main-wrap',
            CLASSVISIBLE: 'blocksvisible',
            CLASSHIDDEN: 'blockshidden'
        },
        BLOCKREGIONSTOHIDE = [
            '#region-pre',
            '#region-post',
        ...
        ALTSTR = {
            ...
            HIDEALLBLOCKS: hideallblocks
        }
        node = null,
        hideallblocks = {
            init: function(Y, node) {
                node.on('click', this.toggle, this);
                this.node = node;
                this.updatelink();
            },
            toggle: function(e) {
                // Check current state
                if (this.node.hasClass(SELECTORS.CLASSVISIBLE)) {
                    this.hideblocks(e);
                } else if (this.node.hasClass(SELECTORS.CLASSHIDDEN)) {
                    this.showblocks(e);
                }
            },
            hideblocks: function(e) {
                e.preventDefault();
                this.node.addClass(SELECTORS.CLASSHIDDEN);
                this.node.removeClass(SELECTORS.CLASSVISIBLE);
                this.node.setAttribute('title', ALTSTR.SHOWALLBLOCKS);
                this.node.one('img').setAttrs({
                    'alt' : ALTSTR.SHOWALLBLOCKS,
                    'src' : M.util.image_url(IMG.SHOW)
                });
                for (var i = 0; i < BLOCKREGIONSTOHIDE.length; i++) {
                    var regionnode = Y.all(BLOCKREGIONSTOHIDE[i]);
                    if (regionnode) {
                       regionnode.addClass('hide');
                    }
                };
                for (var i = 0; i < REGIONSTOEXPAND.length; i++) {
                    var regionnode = Y.all(REGIONSTOEXPAND[i]);
                    if (regionnode) {
                       regionnode.addClass('fullpageview');
                    }
                };
                this.updatelink();
                M.util.set_user_preference('hideallblocks',1);
            },
            showblocks: function(e) {
                var regionavailable = false;
                this.node.addClass(SELECTORS.CLASSVISIBLE);
                this.node.removeClass(SELECTORS.CLASSHIDDEN);
                this.node.setAttribute('title', ALTSTR.HIDEALLBLOCKS);
                this.node.one('img').setAttrs({
                    'alt' : ALTSTR.HIDEALLBLOCKS,
                    'src' : M.util.image_url(IMG.HIDE)
                });
                for (var i = 0; i < BLOCKREGIONSTOHIDE.length; i++) {
                    var regionnode = Y.one(BLOCKREGIONSTOHIDE[i]);
                    if (regionnode != undefined) {
                       regionnode.removeClass('hide');
                       regionavailable = true;
                    }
                };
                for (var i = 0; i < REGIONSTOEXPAND.length; i++) {
                    var regionnode = Y.one(REGIONSTOEXPAND[i]);
                    if (regionnode != undefined) {
                       regionnode.removeClass('fullpageview');
                    }
                };
                // If region avaialble then use JS, else let page reload.
                if (regionavailable) {
                    e.preventDefault();
                    this.updatelink();
                    M.util.set_user_preference('hideallblocks', 0);
                }
            },
            updatelink: function () {
                var oldval = 0;
                var newval = 1;
                if (this.node.hasClass(SELECTORS.CLASSHIDDEN)) {
                    oldval = 1;
                    newval = 0;
                }

                var href = this.node.getAttribute('href').replace('bui_hideallblocks='+oldval, 'bui_hideallblocks='+newval);
                this.node.setAttribute('href', href);
            }
        };
        hideallblocks.init(Y, hideblocknode);
    });
};