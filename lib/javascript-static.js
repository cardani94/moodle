 ...
 * @param {INT} timeleft
 */
M.util.maintenance_mode_timer = function(Y, timeleft) {
    var maintenancenode = Y.one('.box.maintenancewarning'),
        time = timeleft,
        ...
        displayRemainingTime = function(seconds) {
            if (seconds <= 0) {
                maintenancenode.setContent(M.str.admin.sitemaintenance);
            } else {
                var text = M.str.admin.maintenancemodeisscheduled;
                text = text.replace("{$a->sec}", seconds % 60);
                text = text.replace("{$a->min}", Math.floor(seconds / 60));
                maintenancenode.setContent(text);
            }
            // Set error class to highlight the importance.
            if (seconds < 30) {
                maintenancenode.addClass('error');
                maintenancenode.removeClass('warning');
            } else {
                maintenancenode.addClass('warning');
                maintenancenode.removeClass('error');
            }
        }

    if (maintenancenode) {
        maintenancenode.setAttribute('aria-live', 'polite');
        updateTimer();
    }
};
