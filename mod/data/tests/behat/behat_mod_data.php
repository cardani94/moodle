...
class behat_mod_data extends behat_base {
    ...
    public function i_add_a_field_to_database_and_i_fill_the_form_with($fieldtype, $activityname, TableNode $fielddata) {
        ...
        $this->execute_step("behat_general::click_link", $this->escape($activityname),
            true, true);
        $this->execute_step("behat_general::click_link", get_string('fields', 'mod_data'),
            true, true);

        $this->execute_step('behat_forms::i_set_the_field_to', array('newtype', $this->escape($fieldtype)),
            false, false);
        ...
        $this->execute_step("behat_forms::i_set_the_following_fields_to_these_values", $fielddata,
            true, true);
        $this->execute_step('behat_forms::press_button', get_string('add'),
            false, false);
    }
    ...
    public function i_add_an_entry_to_database_with($activityname, TableNode $entrydata) {
        ...
        $this->execute_step("behat_general::click_link", $this->escape($activityname),
            true, true);
        $this->execute_step("behat_general::click_link", get_string('add', 'mod_data'),
            true, true);

        $this->execute_step("behat_forms::i_set_the_following_fields_to_these_values", $entrydata,
            false, false);
    }
}
