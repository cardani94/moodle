...
class behat_mod_quiz extends behat_question_base {
    ...
    public function i_add_question_to_the_quiz_with($questiontype, $quizname, TableNode $questiondata) {
        ...
        $this->execute_step('behat_general::click_link', $quizname, true, true);

        $this->execute_step("behat_navigation::i_navigate_to_node_in",
            array($editquiz, $quizadmin),
            true, true);

        $this->execute_step("behat_general::i_click_on", array($menuxpath, "xpath_element"),
            true, true);
        $this->execute_step("behat_general::i_click_on", array($itemxpath, "xpath_element"),
            true, true);
    ...
    public function i_set_the_max_mark_for_quiz_question($questionname, $newmark) {
        $this->execute_step('behat_general::click_link', $this->escape(get_string('editmaxmark', 'quiz')), true, true);

        $this->execute_step('behat_general::wait_until_exists', array("li input[name=maxmark]", "css_element"),
            false, false);

        $this->execute_step('behat_general::assert_page_contains_text', $this->escape(get_string('edittitleinstructions')));

        $this->execute_step('behat_forms::i_set_the_field_to', array('maxmark', $this->escape($newmark) . chr(10)),
            false, false);
    }
    ...
    public function i_should_see_on_quiz_page($questionname, $pagenumber) {
        ...
        $this->execute_step('behat_general::should_exist', array($xpath, 'xpath_element'), false, false);
    }
    ...
    public function i_should_not_see_on_quiz_page($questionname, $pagenumber) {
        ...
        $this->execute_step('behat_general::should_not_exist', array($xpath, 'xpath_element'), false, false);
    }
    ...
    public function i_click_on_the_page_break_icon_after_question($addorremoves, $questionname) {
        ...
        $this->execute_step("behat_general::i_click_on", array($xpath, "xpath_element"),
            false, false);
    }
    ...
    public function i_move_question_after_item_by_clicking_the_move_icon($questionname, $target) {
        ...
        $this->execute_step("behat_general::i_click_on", array($iconxpath, "xpath_element"),
            true, true);
        $this->execute_step("behat_general::i_click_on", array($this->escape($target), "text"),
            false, false);
    }
    ...
    public function i_delete_question_by_clicking_the_delete_icon($questionname) {
        ...
        $this->execute_step("behat_general::i_click_on", array($slotxpath . $deletexpath, "xpath_element"),
            true, true);
    ...
    public function i_set_the_section_heading_for($sectionname, $sectionheading) {
        $this->execute_step('behat_general::click_link', $this->escape("Edit heading '{$sectionname}'"), true, true);

        $this->execute_step('behat_general::assert_page_contains_text', $this->escape(get_string('edittitleinstructions')),
            false, false);

        $this->execute_step('behat_forms::i_set_the_field_to', array('section', $this->escape($sectionheading) . chr(10)),
            false, false);
    }
